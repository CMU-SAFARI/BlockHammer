...
DDR4::DDR4(const Config& configs, Org org, Speed speed)
    : org_entry(org_table[int(org)]),
    speed_entry(speed_table[int(speed)]),
    read_latency(speed_entry.nCL + speed_entry.nBL)
{
    update_timings(configs);
}

DDR4::DDR4(const Config& configs, const string& org_str, const string& speed_str) :
    DDR4(configs, org_map[org_str], speed_map[speed_str])
{
}

DDR4::DDR4(const Config& configs) : DDR4(configs, org_map[configs["org"]], speed_map[configs["speed"]]){
...
{
    // ACT
    prereq[int(Level::Bank)][int(Command::ACT)] = [] (DRAM<DDR4>* node, Command cmd, int id) {
      switch (int(node->state)) {
        case int(State::Closed): 
          return cmd;
        case int(State::Opened):
          if (node->row_state.find(id) != node->row_state.end()) return Command::MAX;
    ...
    prereq[int(Level::Bank)][int(Command::WR)] = [] (DRAM<DDR4>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return Command::ACT;
            case int(State::Opened):
                if (node->row_state.find(id) != node->row_state.end())
...
void DDR4::init_rowhit()
{
    ...
    // WR
    rowhit[int(Level::Bank)][int(Command::WR)] = [] (DRAM<DDR4>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return false;
            case int(State::Opened):
                if (node->row_state.find(id) != node->row_state.end())
...
void DDR4::init_rowopen()
{
    ...
    // WR
    rowopen[int(Level::Bank)][int(Command::WR)] = [] (DRAM<DDR4>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return false;
            case int(State::Opened): return true;
            default: assert(false);
...
void DDR4::init_timing()
{
    ...
    // CAS <-> CAS
    t[int(Command::RD)].push_back({Command::RD, 1, s.nBL, false});
    t[int(Command::RD)].push_back({Command::RDA, 1, s.nBL, false});
    t[int(Command::RDA)].push_back({Command::RD, 1, s.nBL, false});
    t[int(Command::RDA)].push_back({Command::RDA, 1, s.nBL, false});
    t[int(Command::WR)].push_back({Command::WR, 1, s.nBL, false});
    t[int(Command::WR)].push_back({Command::WRA, 1, s.nBL, false});
    t[int(Command::WRA)].push_back({Command::WR, 1, s.nBL, false});
    t[int(Command::WRA)].push_back({Command::WRA, 1, s.nBL, false});
    ...
    // CAS <-> CAS
    t[int(Command::RD)].push_back({Command::RD, 1, s.nCCDS, false});
    t[int(Command::RD)].push_back({Command::RDA, 1, s.nCCDS, false});
    t[int(Command::RDA)].push_back({Command::RD, 1, s.nCCDS, false});
    t[int(Command::RDA)].push_back({Command::RDA, 1, s.nCCDS, false});
    t[int(Command::WR)].push_back({Command::WR, 1, s.nCCDS, false});
    t[int(Command::WR)].push_back({Command::WRA, 1, s.nCCDS, false});
    t[int(Command::WRA)].push_back({Command::WR, 1, s.nCCDS, false});
    t[int(Command::WRA)].push_back({Command::WRA, 1, s.nCCDS, false});
    t[int(Command::RD)].push_back({Command::WR, 1, s.nCL + s.nBL + 2 - s.nCWL, false});
    t[int(Command::RD)].push_back({Command::WRA, 1, s.nCL + s.nBL + 2 - s.nCWL, false});
    t[int(Command::RDA)].push_back({Command::WR, 1, s.nCL + s.nBL + 2 - s.nCWL, false});
    t[int(Command::RDA)].push_back({Command::WRA, 1, s.nCL + s.nBL + 2 - s.nCWL, false});
    t[int(Command::WR)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nWTRS, false});
    t[int(Command::WR)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nWTRS, false});
    t[int(Command::WRA)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nWTRS, false});
    t[int(Command::WRA)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nWTRS, false});
    ...
    t[int(Command::RD)].push_back({Command::PREA, 1, s.nRTP, false});
    t[int(Command::WR)].push_back({Command::PREA, 1, s.nCWL + s.nBL + s.nWR, false});

    // CAS <-> PD
    t[int(Command::RD)].push_back({Command::PDE, 1, s.nCL + s.nBL + 1, false});
    t[int(Command::RDA)].push_back({Command::PDE, 1, s.nCL + s.nBL + 1, false});
    t[int(Command::WR)].push_back({Command::PDE, 1, s.nCWL + s.nBL + s.nWR, false});
    t[int(Command::WRA)].push_back({Command::PDE, 1, s.nCWL + s.nBL + s.nWR + 1, false}); // +1 for pre
    t[int(Command::PDX)].push_back({Command::RD, 1, s.nXP, false});
    t[int(Command::PDX)].push_back({Command::RDA, 1, s.nXP, false});
    t[int(Command::PDX)].push_back({Command::WR, 1, s.nXP, false});
    t[int(Command::PDX)].push_back({Command::WRA, 1, s.nXP, false});

    // CAS <-> SR: none (all banks have to be precharged)

    // RAS <-> RAS
    t[int(Command::ACT)].push_back({Command::ACT, 1, s.nRRDS, false});
    //t[int(Command::ACT)].push_back({Command::ACT, 4, s.nFAW, false});
    t[int(Command::ACT)].push_back({Command::ACT, 4, s.nFAW, false});
    t[int(Command::ACT)].push_back({Command::PREA, 1, s.nRAS, false});
    t[int(Command::PREA)].push_back({Command::ACT, 1, s.nRP, false});

    // RAS <-> REF
    t[int(Command::PRE)].push_back({Command::REF, 1, s.nRP, false});
    t[int(Command::PREA)].push_back({Command::REF, 1, s.nRP, false});
    t[int(Command::RDA)].push_back({Command::REF, 1, s.nRTP + s.nRP, false});
    t[int(Command::WRA)].push_back({Command::REF, 1, s.nCWL + s.nBL + s.nWR + s.nRP, false});
    t[int(Command::REF)].push_back({Command::ACT, 1, s.nRFC, false});
    // Atb: not worrying about this since we won't have refresh?

    // RAS <-> PD
    t[int(Command::ACT)].push_back({Command::PDE, 1, 1, false});
    t[int(Command::PDX)].push_back({Command::ACT, 1, s.nXP, false});
    t[int(Command::PDX)].push_back({Command::PRE, 1, s.nXP, false});
    t[int(Command::PDX)].push_back({Command::PREA, 1, s.nXP, false});

    // RAS <-> SR
    t[int(Command::PRE)].push_back({Command::SRE, 1, s.nRP, false});
    t[int(Command::PREA)].push_back({Command::SRE, 1, s.nRP, false});
    t[int(Command::SRX)].push_back({Command::ACT, 1, s.nXS, false});

    // REF <-> REF
    t[int(Command::REF)].push_back({Command::REF, 1, s.nRFC, false});

    // REF <-> PD
    t[int(Command::REF)].push_back({Command::PDE, 1, 1, false});
    t[int(Command::PDX)].push_back({Command::REF, 1, s.nXP, false});

    // REF <-> SR
    t[int(Command::SRX)].push_back({Command::REF, 1, s.nXS, false});

    // PD <-> PD
    t[int(Command::PDE)].push_back({Command::PDX, 1, s.nPD, false});
    t[int(Command::PDX)].push_back({Command::PDE, 1, s.nXP, false});

    // PD <-> SR
    t[int(Command::PDX)].push_back({Command::SRE, 1, s.nXP, false});
    t[int(Command::SRX)].push_back({Command::PDE, 1, s.nXS, false});

    // SR <-> SR
    t[int(Command::SRE)].push_back({Command::SRX, 1, s.nCKESR, false});
    t[int(Command::SRX)].push_back({Command::SRE, 1, s.nXS, false});

    /*** Bank Group ***/
    t = timing[int(Level::BankGroup)];
    // CAS <-> CAS
    t[int(Command::RD)].push_back({Command::RD, 1, s.nCCDL, false});
    t[int(Command::RD)].push_back({Command::RDA, 1, s.nCCDL, false});
    t[int(Command::RDA)].push_back({Command::RD, 1, s.nCCDL, false});
    t[int(Command::RDA)].push_back({Command::RDA, 1, s.nCCDL, false});
    t[int(Command::WR)].push_back({Command::WR, 1, s.nCCDL, false});
    t[int(Command::WR)].push_back({Command::WRA, 1, s.nCCDL, false});
    t[int(Command::WRA)].push_back({Command::WR, 1, s.nCCDL, false});
    t[int(Command::WRA)].push_back({Command::WRA, 1, s.nCCDL, false});
    t[int(Command::WR)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nWTRL, false});
    t[int(Command::WR)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nWTRL, false});
    t[int(Command::WRA)].push_back({Command::RD, 1, s.nCWL + s.nBL + s.nWTRL, false});
    t[int(Command::WRA)].push_back({Command::RDA, 1, s.nCWL + s.nBL + s.nWTRL, false});

    // RAS <-> RAS
    t[int(Command::ACT)].push_back({Command::ACT, 1, s.nRRDL, false});

    /*** Bank ***/
    t = timing[int(Level::Bank)];

    // CAS <-> RAS
    t[int(Command::ACT)].push_back({Command::RD, 1, s.nRCD, false});
    t[int(Command::ACT)].push_back({Command::RDA, 1, s.nRCD, false});
    t[int(Command::ACT)].push_back({Command::WR, 1, s.nRCD, false});
    t[int(Command::ACT)].push_back({Command::WRA, 1, s.nRCD, false});

    t[int(Command::RD)].push_back({Command::PRE, 1, s.nRTP, false});
    t[int(Command::WR)].push_back({Command::PRE, 1, s.nCWL + s.nBL + s.nWR, false});

    t[int(Command::RDA)].push_back({Command::ACT, 1, s.nRTP + s.nRP, false});
    t[int(Command::WRA)].push_back({Command::ACT, 1, s.nCWL + s.nBL + s.nWR + s.nRP, false});

    // RAS <-> RAS
    t[int(Command::ACT)].push_back({Command::ACT, 1, s.nRC, false});
    t[int(Command::ACT)].push_back({Command::PRE, 1, s.nRAS, false});
    t[int(Command::PRE)].push_back({Command::ACT, 1, s.nRP, false});
}
