...
public:
    ...
    enum class Org;
    enum class Speed;
    DDR4(const Config& configs, Org org, Speed speed);
    DDR4(const Config& configs, const string& org_str, const string& speed_str);
    ...
    void update_timings(const Config& configs){
        // vector<TimingEntry> timing[int(Level::MAX)][int(Command::MAX)]
        for (int lvl = 0 ; lvl < int(Level::MAX) ; lvl++){
            this->timing[lvl]->erase(this->timing[lvl]->begin(), this->timing[lvl]->end());
        }
        init_speed();
        init_prereq();
