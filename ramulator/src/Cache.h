...
protected:
  ...
  ScalarStat cache_hammer_access;
...
public:
  ...
  Cache(int size, int assoc, int block_size, int mshr_entry_num,
      Level level, std::shared_ptr<CacheSystem> cachesys, vector<float> *throttling_coeffs_ptr);
...
protected:
  ...
  // needed.
  bool need_eviction(const std::list<Line>& lines, long addr, bool bypass);

  // @Giray: bypass is a flag I use for always missing cachelines even if 
  // the data is cached. It is a quick hack to generate RowHammer accesses 
  // by somehow bypassing the caches. 
...
public:
  CacheSystem(const Config& configs, std::function<bool(Request)> send_memory,
    ...
    send_memory(send_memory), upgrade_prefetch_req_in_mem(upgrade_prefetch_req)
       ,throttling_coeffs(throttling_coeffs)	{
