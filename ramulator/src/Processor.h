...
public:
    ...
    bool get_unfiltered_request(long& bubble_cnt, long& req_addr, Request::Type& req_type, string& payload);
...
private:
    ...
    string payload; //carries any additional info a req has in the rest of the line.
    bool more_reqs;
    long last = 0;
...
public:
    Processor(const Config& configs, vector<std::string> trace_list,
        function<bool(Request)> send, function<bool(long)> upgrade_prefetch_req, MemoryBase& memory, vector<float> &throttling_coeffs);
    void tick();
    void receive(Request& req);
    void reset_stats();
    bool finished();
    ...
    bool is_core_exempted_from_limit_instr(int core_id);
    vector<float> * throttling_coeffs_ptr;
    ...
    long expected_limit_cycles = 0;
    int core_limit_instr_exempt_flags = 0;
    int l3_size =  1 << 24;
    int l3_assoc = 1 << 3;
    int l3_blocksz = 1 << 6;
